...
description: Java Stage1 单类三维审计器（sinks+routes+auth_markers 一次输出）。当 worker 需要对某个 class 做审计时，必须用 subagent 调用本技能：先枚举 methods/fields 粗筛，再按需 get_method_by_name 拉取少量方法源码；最后复用现有 sink/route/auth 三个技能输出严格 schema 的 sinks/routes/auth_markers，并给出 code_context_level 供 worker 落库。禁止跨文件搜索与推测未提供内容。
---
...
本技能的职责是：在同一个 subagent 上下文中依次执行三次分析，并把三者输出合并为一次 JSON 返回，减少 worker 的推测与胶水逻辑。
