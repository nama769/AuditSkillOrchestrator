...
description: Java Stage1 审计状态检测器（Postgres）。当用户询问“当前审计进度/覆盖率/卡在哪/哪些页未完成/有哪些告警/如何继续审计”，或主 agent 需要在启动前/运行中恢复现场时，必须使用本技能从 Postgres（javaAudit）查询 stage1_runs/pages/classes/warnings/sinks/routes/auth_markers 并输出进度报告与下一步建议。
---
...
- 如果 expected_class_count 为 null 的页很多：提示可能尚未被 worker 领取/写入 class 清单
